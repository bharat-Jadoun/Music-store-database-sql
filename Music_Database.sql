...
SELECT TOP 1
    ...
    SUM(Total) AS TotalInvoiceAmount
FROM invoice
GROUP BY billing_city
...
SELECT TOP 10
      artist.artist_id, artist.name AS ArtistName,COUNT(artist.artist_id) AS number_of_songs
FROM track
...
FROM
    track
WHERE
    track.milliseconds > (SELECT AVG(milliseconds) FROM track)
ORDER BY
...
ORDER BY
    CustomerFirstName, CustomerLastName, ArtistName;
...
WITH CountryGenreCounts AS (
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY c.country ORDER BY COUNT(i.invoice_id) DESC) AS rank
    FROM
        customer c
    JOIN
...
SELECT country, genre_name FROM CountryGenreCounts
WHERE rank = 1;
...
WITH customter_with_country AS (
    ...
    ORDER BY
        billing_country ASC, first_name DESC
),
